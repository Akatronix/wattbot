...
// --- API Request Helper: Update General Socket Info ---
const updateSocketData = async (id, dataToUpdate) => {
  const API_URL = `${import.meta.env.VITE_BACKEND_URL}/api/sockets/updateData`;
  ...
  const response = await fetch(API_URL, {
    ...
    body: JSON.stringify({ id: id.trim(), ...dataToUpdate }),
  });
...
// --- API Request Helper: Set Max Power ---
const setMaxPowerAPI = async (id, maxPowerValue) => {
  // Using the host from .env and ensuring the ID is trimmed of spaces
  const sanitizedId = id.trim();
  const API_URL = `${import.meta.env.VITE_BACKEND_URL}/api/sockets/setPower/${sanitizedId}`;

  const response = await fetch(API_URL, {
    method: "POST", // Method set to POST as requested
    headers: {
      "Content-Type": "application/json",
      "token": localStorage.getItem("token"),
    },
...
const SetMaxPowerForm = () => {
  ...
  const handleMaxPowerSubmit = async (e) => {
    ...
    // Basic Validation
    if (!socketId.trim() || !maxPowerInput || isNaN(maxPowerInput) || Number(maxPowerInput) < 0) {
      toast.error("Please enter a valid Socket ID and a non-negative Max Power value.");
      setLoading(false);
      return;
    }
    ...
    try {
      await setMaxPowerAPI(socketId, maxPowerInput);
      toast.success(`Max power for socket ${socketId.trim()} set to ${maxPowerInput}W.`);
        ...
        <form onSubmit={handleMaxPowerSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => setSocketId(e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
              placeholder="Paste Socket ID here"
              required
            />
          </div>
          ...
          <div className="pt-4">
            <Button type="submit" disabled={loading} className="bg-green-600 hover:bg-green-700 w-full text-white">
              {loading ? "Setting..." : "Set Max Power"}
            </Button>
          </div>
        </form>
...
// --- Main Component: Update Socket Info ---
const UpdateSocketInfo = ({ initialData }) => {
  const [socketForm, setSocketForm] = useState({
    id: "",
    name: "",
  ...
  const handleSocketSubmit = async (e) => {
    ...
    if (!socketForm.id.trim() || !socketForm.name || !socketForm.location || !socketForm.type) {
      toast.error("Please fill in all fields, including the Socket ID.");
      return;
    }
    ...
    <div className="max-w-4xl mx-auto p-4">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600">
            Update socket details or set power consumption limits.
          </p>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Socket Details</CardTitle>
            <CardDescription>
              Modify the name, location, or type of an existing socket.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSocketSubmit} className="space-y-4">
              ...
              <div className="pt-2">
                <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white w-full md:w-auto" disabled={loading}>
